...
must_haves:
  truths:
    ...
    - "Running viflo init --full where CLAUDE.md already exists only updates the sentinel block — content outside is unchanged"
    ...
    - path: "bin/lib/writers.cjs"
      ...
      exports:
        [
          "writeCLAUDEmd",
          "writeSettingsJson",
          "writePlanningScaffold",
          "writeCLAUDEmdTemplate",
        ]
    - path: "bin/viflo.cjs"
...
<task type="auto">
  ...
  <action>
Add two new exported functions to bin/lib/writers.cjs, appended after the existing writeSettingsJson function. Do NOT modify existing functions — only add new ones.
...
Use `writeIfChanged` for files that don't exist yet. For existing files, skip them without calling `writeIfChanged` at all (check `fs.existsSync` first — if exists, push `{ path, written: false, reason: 'skipped (already exists)' }` and continue).
...
(Minimal section headers only — no placeholder text or instructions, per user decision.)
...
(Minimal valid STATE.md that GSD tools can parse — not empty but structurally complete, per user decision.)
...
<done> - `writePlanningScaffold` and `writeCLAUDEmdTemplate` exported from bin/lib/writers.cjs - `node -e "require('./bin/lib/writers.cjs')"` exits 0 with no errors - Existing exports (writeCLAUDEmd, writeSettingsJson) still present and unchanged
</done>
</task>
...
4. Replace the core logic section with mode-aware logic. Keep the existing `--minimal` behaviour intact. Add `--full` branch:
   ...
   if (hasMinimalFlag && !hasFullFlag) {
     ...
     const claudeStatus = claudeResult.written
       ? claudeResult.reason
       : "skipped";
     const settingsStatus = settingsResult.written
   ...
   if (hasFullFlag) {
     ...
     const allResults = [
       {
         label: "CLAUDE.md",
         written: claudeResult.written,
         reason: claudeResult.reason,
       },
       {
         label: ".claude/settings.json",
         written: settingsResult.written,
     ...
     if (skipped === 0) {
       console.log("");
       console.log(
         "Next: edit .planning/PROJECT.md and run /gsd:new-project to plan your first milestone.",
       );
     }
...
<done> - `viflo init --full [path]` exits 0 on a fresh directory - `.planning/PROJECT.md`, `.planning/STATE.md`, `.planning/ROADMAP.md`, `.planning/config.json` all created - CLAUDE.md created with richer template (project sections + sentinel block) - Running `--full` a second time exits 0 and shows all files as skipped - `viflo init --minimal [path]` still works exactly as before (no regression) - `pnpm test:cli` passes all 35 existing tests
</done>
</task>
...
1. `node bin/viflo.cjs init --full /tmp/verify-full` — check output format and files created
2. `node bin/viflo.cjs init --full /tmp/verify-full` — second run, all skipped
3. `node bin/viflo.cjs init --minimal /tmp/verify-minimal` — confirm minimal still works
   </verification>
